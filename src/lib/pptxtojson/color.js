...
// ADITYANTA: PowerPoint (and LibreOffice) apply tint/shade in linear RGB, not
// on HSL lightness. HSL made e.g. "tx1 tint 75%" far too light/dark.
const toLinear = (c) => {
  const v = c / 255
...
const mapLinear = (rgbStr, fn, isAlpha) => {
  const { r, g, b, a } = tinycolor(rgbStr).toRgb()
  const out = tinycolor({ r: toSrgb(fn(toLinear(r))), g: toSrgb(fn(toLinear(g))), b: toSrgb(fn(toLinear(b))), a })
  return isAlpha ? out.toHex8() : out.toHex()
}

export function applyShade(rgbStr, shadeValue, isAlpha) {
  const f = Math.min(Math.max(shadeValue, 0), 1)
  return mapLinear(rgbStr, (c) => c * f, isAlpha)
}

export function applyTint(rgbStr, tintValue, isAlpha) {
  const f = Math.min(Math.max(tintValue, 0), 1)
  return mapLinear(rgbStr, (c) => 1 - (1 - c) * f, isAlpha)
}
