...
import { getChartInfo, getChartExtras } from './chart'
...
import { findTableStyle, resolveTableCellStyle } from './tableStyle' // ADITYANTA
...
export async function parse(file, options = {}) {
  ...
  // ADITYANTA: use the real slide order from presentation.xml (<p:sldIdLst>),
  // not the file names — reordered decks keep slide1.xml..N.xml names.
  const orderedSlides = await getSlideOrder(zip)
  if (orderedSlides.length) filesInfo.slides = orderedSlides.filter((f) => zip.file(f))
...
async function getSlideOrder(zip) {
  try {
    const pres = await readXmlFile(zip, 'ppt/presentation.xml')
    const rels = await readXmlFile(zip, 'ppt/_rels/presentation.xml.rels')
    let ids = getTextByPathList(pres, ['p:presentation', 'p:sldIdLst', 'p:sldId'])
    let relList = getTextByPathList(rels, ['Relationships', 'Relationship'])
    if (!ids || !relList) return []
    if (ids.constructor !== Array) ids = [ids]
    if (relList.constructor !== Array) relList = [relList]
    const byId = {}
    for (const r of relList) byId[r.attrs.Id] = r.attrs.Target
    return ids
      .map((node) => byId[node.attrs['r:id']])
      .filter(Boolean)
      .map((t) => {
...
// ADITYANTA: master decorations are drawn first (behind), then layout ones,
// each tagged with its origin so the importer can lock them.
async function getLayoutElements(warpObj) {
  ...
  const slideShowMasterSp = getTextByPathList(warpObj['slideContent'], ['p:sld', 'attrs', 'showMasterSp'])

  const collect = async (tree, source) => {
    if (!tree) return
    for (const nodeKey in tree) {
      const list = tree[nodeKey].constructor === Array ? tree[nodeKey] : [tree[nodeKey]]
      for (const item of list) {
        const ph = getTextByPathList(item, ['p:nvSpPr', 'p:nvPr', 'p:ph'])
        if (ph) continue
        const ret = await processNodesInSlide(nodeKey, item, warpObj, source)
        if (ret) {
          ret.__source = source === 'slideMasterBg' ? 'master' : 'layout'
          elements.push(ret)
        }
      }
    }
  ...
  if (slideShowMasterSp !== '0') {
    if (showMasterSp !== '0') await collect(nodesSldMaster, 'slideMasterBg')
    await collect(nodesSldLayout, 'slideLayoutBg')
  }
  return elements
}
...
async function processSpNode(node, warpObj, source, groupHierarchy = []) {
  ...
  // ADITYANTA: PowerPoint inheritance: slide placeholder -> layout placeholder
  // (matched by idx, else by type) -> master placeholder matched by the
  // layout placeholder's *type* (master idx values are unrelated to the
  // layout's, e.g. idx 2 is the date on many masters).
  const isPlaceholder = !!getTextByPathList(node, ['p:nvSpPr', 'p:nvPr', 'p:ph'])
  const layoutTables = warpObj['slideLayoutTables']
  const masterTables = warpObj['slideMasterTables']
  let slideLayoutSpNode
  if (isPlaceholder) {
    if (idx) slideLayoutSpNode = layoutTables['idxTable'][idx]
    if (!slideLayoutSpNode && type) slideLayoutSpNode = layoutTables['typeTable'][type]
    if (!slideLayoutSpNode && type === 'ctrTitle') slideLayoutSpNode = layoutTables['typeTable']['title']
    if (!slideLayoutSpNode && type === 'title') slideLayoutSpNode = layoutTables['typeTable']['ctrTitle']
  }
  const layoutType = getTextByPathList(slideLayoutSpNode, ['p:nvSpPr', 'p:nvPr', 'p:ph', 'attrs', 'type'])
  const masterKey = (t) => {
    if (t === 'ctrTitle' || t === 'title') return 'title'
    if (!t || t === 'obj' || t === 'body' || t === 'subTitle' || t === 'tbl' || t === 'chart' || t === 'dgm' || t === 'media' || t === 'clipArt' || t === 'pic') return 'body'
    return t
  }
  let slideMasterSpNode
  if (isPlaceholder) {
    slideMasterSpNode = masterTables['typeTable'][masterKey(slideLayoutSpNode ? layoutType : type)]
  }
  ...
  if (!type && isPlaceholder) type = layoutType
  if (!type && isPlaceholder) type = 'body'
  if (type === 'obj' && isPlaceholder) type = 'body'
...
async function genShape(node, slideLayoutSpNode, slideMasterSpNode, name, type, order, warpObj, source, link, groupHierarchy = []) {
  ...
  // ADITYANTA: a placeholder without its own xfrm also takes its rotation
  // and flips from the layout/master (e.g. a vertical footer)
  const effectiveXfrmNode = slideXfrmNode || slideLayoutXfrmNode || slideMasterXfrmNode
  const isFlipV = getTextByPathList(effectiveXfrmNode, ['attrs', 'flipV']) === '1'
  const isFlipH = getTextByPathList(effectiveXfrmNode, ['attrs', 'flipH']) === '1'

  const rotate = angleToDegrees(getTextByPathList(effectiveXfrmNode, ['attrs', 'rot']))

  // ADITYANTA: SmartArt drawings give the text its own rotation (dsp:txXfrm
  // rot, relative to the shape). The text's absolute angle is the sum; a
  // quarter turn is the same rectangle with its sides swapped.
  const txtXframeRot = angleToDegrees(getTextByPathList(node, ['p:txXfrm', 'attrs', 'rot'])) || 0
  const txtRotate = (((rotate || 0) + txtXframeRot) % 360 + 360) % 360
  const txtQuarterTurn = ((txtXframeRot % 180) + 180) % 180 === 90
  ...
  let shadow
  const effectLst = getTextByPathList(node, ['p:spPr', 'a:effectLst'])
  let outerShdwNode = getTextByPathList(effectLst, ['a:outerShdw'])
  let shadowPhClr
  if (!effectLst) {
    // ADITYANTA: theme effect style (p:style/a:effectRef), e.g. default shape shadows
    const effectRef = getTextByPathList(node, ['p:style', 'a:effectRef'])
    const style = getThemeStyleEntry(warpObj, 'a:effectStyleLst', Number(getTextByPathList(effectRef, ['attrs', 'idx'])))
    if (style) {
      outerShdwNode = getTextByPathList(style.node, ['a:effectLst', 'a:outerShdw'])
      shadowPhClr = getSolidFill(effectRef, undefined, undefined, warpObj) || undefined
    }
  }
  if (outerShdwNode) shadow = getShadow(outerShdwNode, warpObj, shadowPhClr)
  ...
  const textData = { ...data, type: 'text', isVertical, rotate: txtRotate }
  if (txtQuarterTurn) {
    textData.left = left + (width - height) / 2
    textData.top = top + (height - width) / 2
    textData.width = height
...
async function processPicNode(node, warpObj, source) {
  ...
  let geom = 'rect'
  let custPath
  // ADITYANTA: a picture placeholder without its own geometry takes the
  // layout placeholder's (e.g. a photo with a torn-paper edge)
  let geomSpPr = node['p:spPr']
  if (!geomSpPr?.['a:prstGeom'] && !geomSpPr?.['a:custGeom']) {
    const ph = getTextByPathList(node, ['p:nvPicPr', 'p:nvPr', 'p:ph', 'attrs'])
    const tables = warpObj['slideLayoutTables']
    const layoutNode = ph && ((ph.idx && tables?.idxTable?.[ph.idx]) || (ph.type && tables?.typeTable?.[ph.type]))
    if (layoutNode?.['p:spPr']) geomSpPr = layoutNode['p:spPr']
  }
  const prstGeom = getTextByPathList(geomSpPr, ['a:prstGeom', 'attrs', 'prst'])
  const custGeom = getTextByPathList(geomSpPr, ['a:custGeom'])
  ...
  if (prstGeom) {
    ...
    try {
      custPath = getCustomShapePath(custGeom, width, height) || undefined
...
async function genTable(node, warpObj) {
  ...
  // ADITYANTA: layered table-style resolution (see tableStyle.js)
  const thisTblStyle = findTableStyle(warpObj['tableStyles'], getTblPr['a:tableStyleId'])
  const styleFlags = {
    firstRow: tblStylAttrObj.isFrstRowAttr === 1,
    lastRow: tblStylAttrObj.isLstRowAttr === 1,
    firstCol: tblStylAttrObj.isFrstColAttr === 1,
    lastCol: tblStylAttrObj.isLstColAttr === 1,
    bandRow: tblStylAttrObj.isBandRowAttr === 1,
    bandCol: tblStylAttrObj.isBandColAttr === 1,
  }
  const borders = {}
  ...
  const data = []
  const rowHeights = []
  const nCols = Math.max(colWidths.length, ...trNodes.map((tr) => {
    const tcs = tr['a:tc']
    return Array.isArray(tcs) ? tcs.length : (tcs ? 1 : 0)
  }))
  for (let i = 0; i < trNodes.length; i++) {
    ...
    for (let j = 0; j < tcNodes.length; j++) {
      ...
      const st = resolveTableCellStyle(thisTblStyle, styleFlags, i, j, trNodes.length, nCols, warpObj)
      const text = genTextBody(tcNode['a:txBody'], tcNode, undefined, undefined, undefined, warpObj)
      // explicit cell formatting (tcPr) only; style parts are resolved above
      const cell = await getTableCellParams(tcNode, undefined, undefined, warpObj)
      const td = { text }
      ...
      const bold = cell.fontBold ?? st.bold
      if (bold) td.fontBold = true
      if (st.italic) td.fontItalic = true
      const fontColor = cell.fontColor || st.fontColor
      if (fontColor) td.fontColor = fontColor
      let fill = cell.fillColor
      if (!fill && getTextByPathList(tcNode, ['a:tcPr', 'a:noFill'])) fill = null
      if (fill === undefined) fill = st.fill
      if (fill) td.fillColor = fill
      td.borders = { ...st.borders, ...(cell.borders || {}) }
      tr.push(td)
    }
    data.push(tr)
  }
...
async function genChart(node, warpObj) {
  ...
  // ADITYANTA: title, legend, fonts, axes, data labels
  data.extras = getChartExtras(content, plotArea, chart, warpObj)
