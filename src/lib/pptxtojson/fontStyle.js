...
function appendMasterTextStyleNodes(styleNodes, type, lvl, slideMasterTextStyles) {
  ...
  // ADITYANTA: subtitle/content placeholders inherit from the master body style
...
// ADITYANTA: all caps / small caps (a:rPr cap="all" | "small")
export function getFontCaps(node, pNode, textBodyNode, slideLayoutSpNode, slideMasterSpNode, type, slideMasterTextStyles, lvl) {
  const styleNodes = getFontStyleNodes(node, pNode, textBodyNode, slideLayoutSpNode, slideMasterSpNode, type, slideMasterTextStyles, lvl)
  const cap = getFontAttr(styleNodes, 'cap')
  if (cap === 'all') return 'text-transform: uppercase;'
  if (cap === 'small') return 'font-variant: small-caps;'
  return ''
}
