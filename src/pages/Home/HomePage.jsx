...
const navItems = [
    ...
    const loadData = async () => {
      if (isMounted) {
        ...
        try {
          ...
          // store_mode=non_store: only user uploads / non-store templates on the home Templates tab
          const params = { storeMode: 'non_store' }
  ...
  // Fetch store templates the first time user visits the Store tab.
  // The fetchTemplates() call inside AppContext already has its own 8-second
  // dedupe + in-flight guard, so revisits to this tab won't refire the request.
  useEffect(() => {
    if (activeTab !== 'store') return
    fetchTemplates({ storeMode: 'store' }).catch((err) => {
      logger.error('Store templates fetch error:', err)
    })
  ...
  // pills derived from the response (only topics with >= 1 template), and the
  // currently visible templates after applying the selected pill + search.
  const storeTemplatesVisible = useMemo(() => {
    return (apiStoreTemplates || []).filter((t) => !shouldHideTemplate(t))
  }, [apiStoreTemplates])

  const storeTopics = useMemo(() => {
    // Build the canonical list of allowed topic labels (the 16 product wants).
    const canonical = [
      'Mathematics',
      'Science',
      'History',
  ...
  const storeDisplayItems = useMemo(() => {
    if (selectedStoreSubTopic) {
      return filteredStoreTemplates.filter(
        (t) => (t.sub_topic || '').trim().toLowerCase() === selectedStoreSubTopic.toLowerCase()
      )
    }
    ...
    for (const t of filteredStoreTemplates) {
      const sub = (t.sub_topic || '').trim()
      if (sub) {
    ...
    const folderList = Object.values(groups).sort((a, b) => a.name.localeCompare(b.name))
    return [...folderList, ...items]
  }, [filteredStoreTemplates, selectedStoreSubTopic])

  useEffect(() => {
    setSelectedStoreSubTopic(null)
  }, [selectedStoreTopic, searchQuery])

  // Reset store topic if it disappears from the available pills (e.g. after a refresh)
  useEffect(() => {
    if (selectedStoreTopic !== 'All' && !storeTopics.includes(selectedStoreTopic)) {
      setSelectedStoreTopic('All')
    }
  }, [storeTopics, selectedStoreTopic])
  ...
  const handleToggleFavorite = async (template, e) => {
    ...
    const templateId = getTemplateId(template)
    if (!templateId) {
      toast.error('Unable to bookmark this template')
      return
    }
    try {
      let result
      if (isTemplateFavorite(template)) {
        result = await removeFavorite(templateId)
        if (result?.success === false) throw new Error(result?.error || 'Failed to remove bookmark')
        toast.success('Removed from bookmarks')
      } else {
        result = await addFavorite(templateId, template)
        if (result?.success === false) throw new Error(result?.error || 'Failed to add bookmark')
  ...
  const FolderCard = ({ folder }) => {
    return (
      <div
        onClick={() => setSelectedStoreSubTopic(folder.name)}
        className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer border border-gray-100 flex flex-col h-full group"
      >
        <div className="h-44 bg-gradient-to-br from-amber-50 to-orange-100 relative flex items-center justify-center overflow-hidden">
          {/* Subtle grid pattern background */}
          <div className="absolute inset-0 opacity-10 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px]" />
          
          {/* Elegant folder SVG with animation */}
          <div className="relative z-10 transition-transform duration-300 group-hover:scale-110">
            ...
            >
              <path
                d="M2 17.5C2 15.0147 2 13.772 2.73223 13.0036C3.46447 12.2351 4.64778 12.1862 7.0144 12.0884L7.54583 12.0663C8.75138 12.016 9.35416 11.9909 9.87062 11.696C10.3871 11.4012 10.763 10.8705 11.5147 9.80897L11.8845 9.28695C12.4339 8.5113 12.7086 8.12347 13.0903 7.89736C13.4721 7.67124 13.9143 7.63228 14.7989 7.55437L16.9856 7.36173C19.3522 7.15328 20.5355 7.04905 21.2678 7.76566C22 8.48227 22 9.6433 22 11.9654V17.5C22 19.8562 22 21.0343 21.2678 21.7672C20.5355 22.5 19.3522 22.5 16.9856 22.5H7.0144C4.64778 22.5 3.46447 22.5 2.73223 21.7672C2 21.0343 2 19.8562 2 17.5Z"
                fill="url(#folderGrad)"
                stroke="url(#folderStroke)"
                strokeWidth="1.5"
              />
              <defs>
                <linearGradient id="folderGrad" x1="2" y1="7.36" x2="22" y2="22.5" gradientUnits="userSpaceOnUse">
                  <stop offset="0%" stopColor="#FBBF24" />
                  <stop offset="100%" stopColor="#F97316" />
                </linearGradient>
                <linearGradient id="folderStroke" x1="2" y1="7.36" x2="22" y2="22.5" gradientUnits="userSpaceOnUse">
            ...
            {/* Number badge */}
            <span className="absolute -top-1.5 -right-1.5 bg-orange-600 text-white text-[11px] font-bold px-2 py-0.5 rounded-full shadow-sm">
              {folder.templates.length}
            </span>
          </div>

          <div className="absolute top-4 left-6 w-16 h-16 rounded-full bg-orange-200/20 blur-xl pointer-events-none" />
          <div className="absolute bottom-4 right-6 w-20 h-20 rounded-full bg-amber-200/30 blur-xl pointer-events-none" />
        </div>
        <div className="p-4 flex-grow flex flex-col justify-between">
          <div>
            <h4 className="font-semibold text-gray-900 mb-1 group-hover:text-orange-600 transition-colors truncate">
              {folder.name}
            </h4>
          ...
          <div className="flex items-center gap-1.5 mt-3 text-xs font-semibold text-orange-600 group-hover:translate-x-1 transition-transform">
  ...
  return (
    <div className="min-h-screen bg-gray-50 relative">
      ...
      <header className="relative z-20 w-full px-4 sm:px-8 md:px-12 lg:px-24 xl:px-[200px] py-6 lg:py-12">
      ...
      <div className="relative z-10 my-4 lg:my-8 mx-4 sm:mx-8 md:mx-12 lg:mx-24 xl:mx-[200px]">
        <div className="bg-white rounded-2xl lg:rounded-[32px] flex flex-col lg:flex-row shadow-sm p-4 sm:p-6 lg:p-12 gap-6 lg:gap-12">
          ...
          <aside className="w-full lg:w-[232px] lg:flex-shrink-0">
            ...
            {isPremium ? (
              <div className="mb-6 bg-gradient-to-r from-green-100 to-emerald-50 rounded-xl p-3 -mx-1">
                <p className="text-sm text-green-800/70 mb-1">Membership Status</p>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-green-900">Premium Member</span>
                </div>
              </div>
            ) : (
              <div className="mb-6 bg-gradient-to-r from-orange-100 to-amber-50 rounded-xl p-3 -mx-1 flex items-center justify-between">
                <div>
                  <p className="text-xs text-orange-800/70 font-medium">Free Member</p>
                  <p className="text-xs text-gray-600 font-semibold">Standard Plan</p>
                </div>
                <button
                  onClick={() => setShowUpgradeModal(true)}
                  className="flex items-center gap-1 text-xs font-bold bg-orange-500 text-white px-2.5 py-1.5 rounded-lg hover:bg-orange-600 transition-colors shadow-sm"
                >
          ...
          <main className="flex-1" style={{ minWidth: 0 }}>
            ...
            {activeTab === 'store' && (
              !isPremium ? (
                <div className="bg-gradient-to-br from-orange-50 via-white to-amber-50 rounded-3xl p-10 border-2 border-orange-100 shadow-xl flex flex-col items-center justify-center text-center my-6">
                  ...
                  <span className="bg-orange-100 text-orange-600 text-xs font-bold px-3.5 py-1.5 rounded-full uppercase tracking-wider mb-4">
                    Subscribed Members Only
                  </span>
                  <h3 className="text-3xl font-extrabold text-gray-900 mb-3">
                    Unlock Exclusive Store Templates
                  </h3>
                  <p className="text-gray-600 max-w-md mb-8 text-base leading-relaxed">
                    Store items are available exclusively for Subscribed Members. Upgrade your membership to get instant access to premium presentation templates and assets.
                  </p>
                  <button
                    onClick={() => setShowUpgradeModal(true)}
                    className="px-8 py-4 bg-orange-500 hover:bg-orange-600 text-white font-bold text-lg rounded-2xl shadow-lg hover:shadow-orange-200 transition-all flex items-center gap-3 transform hover:-translate-y-0.5"
                  >
                ...
                <>
                  {/* Topic pills - horizontal scrollable row */}
                  <div className="mb-6 -mx-2">
                    <div
                      className="flex items-center gap-3 overflow-x-auto px-2 pb-2"
                      style={{ scrollbarWidth: 'thin' }}
                    >
                      {storeTopics.map((topic) => {
                        const isActive = selectedStoreTopic === topic
                        const label = topic === 'All' ? 'ALL' : topic
                        return (
                          <button
                            key={topic}
                            onClick={() => setSelectedStoreTopic(topic)}
                            className={`flex-shrink-0 px-5 py-2.5 rounded-full text-sm font-semibold whitespace-nowrap transition-all border ${
                              isActive
                  ...
                  {selectedStoreSubTopic && (
                    <div className="mb-6 flex flex-col gap-2">
                      <button
                        onClick={() => setSelectedStoreSubTopic(null)}
                        className="self-start flex items-center gap-1.5 text-sm font-semibold text-orange-600 hover:text-orange-700 transition-colors group"
                      >
                        <svg
                          width="16"
                          height="16"
                        ...
                        Back to {selectedStoreTopic === 'All' ? 'Store' : selectedStoreTopic}
                      </button>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <h3 className="text-xl font-bold text-gray-900">
                          {selectedStoreSubTopic} <span className="text-sm font-normal text-gray-500">({storeDisplayItems.length} {storeDisplayItems.length === 1 ? 'template' : 'templates'})</span>
                        </h3>
                      </div>
                    </div>
                  )}

                  {/* Store templates grid */}
                  {isLoadingStoreTemplates && storeDisplayItems.length === 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                      <Skeleton variant="card" count={6} />
                    </div>
                  ) : storeDisplayItems.length === 0 ? (
                    <EmptyState
                      icon="search"
                      title="No templates found"
                      description={
                        selectedStoreSubTopic
                          ? `No templates found in folder "${selectedStoreSubTopic}".`
                          : selectedStoreTopic === 'All'
                          ? 'No store templates are available right now.'
                          : `No templates found for "${selectedStoreTopic}". Try another topic.`
                      }
                    />
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {trashedItems.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    ...
                    {trashedItems.map((item) => {
                      ...
                      const daysDiff = Number.isNaN(deletedAt.getTime()) ? 0 : Math.floor((new Date() - deletedAt) / (1000 * 60 * 60 * 24))
                      const daysRemaining = Math.max(0, 15 - daysDiff)
                      const displayTitle = typeof item.title === 'string'
                        ? item.title
                        : (typeof item.name === 'string' ? item.name : 'Untitled Project')
                      const displayFrames = typeof item.frameCount === 'number'
                        ? item.frameCount
                        : (Array.isArray(item.frames) ? item.frames.length : (Number(item.frames) || 1))
                        ...
                        <div key={item.id} className={`flex items-center justify-between p-4 bg-white rounded-xl border-2 transition-all ${isSelected ? 'border-red-300 bg-red-50/30' : 'border-gray-200 hover:border-gray-300'}`}>
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">{displayFrames} frames &bull; <span className={daysRemaining <= 3 ? 'text-red-500 font-medium' : ''}>{daysRemaining} days left</span></p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
