...
const apiCall = async (endpoint, options = {}, retryCount = 0, isRateLimited = false) => {
  ...
  try {
    ...
    if (!contentType || !contentType.includes('application/json')) {
      ...
      if (rawText.includes('ERR_NGROK') || rawText.includes('is offline')) {
        throw new Error('Backend tunnel is offline (ERR_NGROK_3200). Please restart your backend tunnel.')
      }
      throw new Error(`Invalid response from server (${response.status}): Expected JSON, got ${contentType || 'HTML/plain text'}. Backend might be unreachable.`)
    }
...
export const userAPI = {
  ...
  // Payload: { plan_id, plan, autopay, success_url }
  // Returns: { success, payment_id, payment_link }
  buyMembership: (plan, successUrl = null, autopay = false, planId = null) => {
    const payload = {}
    if (planId) {
